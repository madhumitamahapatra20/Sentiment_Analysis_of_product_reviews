...
class DataCollection:
	...
	def visualiazation(self, dataframe=None, filename=None):
		'''
		it generates and saves the visualization image into wc_folder,
		extract all the comments and then feature extraction using countvectorizer
		and give it to the model for predicting its sentiments, then based on that
		plot a pie chart
		'''
		from sklearn.feature_extraction.text import CountVectorizer
		countvector=pickle.load(open('vector.pkl','rb'))
		pred=countvector.transform(dataframe['Comment Heading']).toarray()
		model = pickle.load(open('model.pkl','rb'))
		predicted_array = model.predict(pred)
		preddf = pd.DataFrame(predicted_array)
		preddf.dropna(inplace = True)
		preddf[0] = preddf[0].replace(-1,0)
		arr = preddf[0].to_numpy()
		unique, counts = np.unique(arr, return_counts=True)
		dictionary = dict(zip(unique, counts))
		if 1 in dictionary.keys():
			positive_count = dictionary.get(1)
		else:
			positive_count = 0

		if 0 in dictionary.keys():
			negative_count = dictionary.get(0)
		else:
			negative_count = 0
		...
		# plt.show()
		plt.legend()
		# create path to save visualized image
		image_path = os.path.join(app.config['IMG_FOLDER'], filename + '.png')
	...
	def save_wordcloud_image(self, dataframe=None, img_filename=None):
		...
		wc = WordCloud(width=600, height=300, background_color='black', stopwords=STOPWORDS).generate(str(txt))
		...
		image_path = os.path.join(app.config['IMG_FOLDER'], img_filename + 'wc.png')
		# Clean previous image from the given path
		CleanCache(directory=app.config['IMG_FOLDER'])
		# save the image file to the image path
		plt.savefig(image_path)
...
def index():
	if request.method == 'POST':
		try:
			...
			for prodName, productLink in product_name_Links[:10]:
...
# route to display visualization
@app.route('/show')  
@cross_origin()
def visualize():
	...
	return render_template("visualize.html", user_image = full_filename)

# route to display wordcloud
@app.route('/showwc')  
@cross_origin()
def show_wordcloud():
	img_file = os.listdir(app.config['IMG_FOLDER'])[1]
	full_filename = os.path.join(app.config['IMG_FOLDER'], img_file)
